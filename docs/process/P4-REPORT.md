# P4 — REPORT & RECONCILE — DRAFT v0.4
...
1. **GATE-10 divergence diff first:** compare git technical statuses / sign-offs vs Jira workflow per FORMATS §10 mapping; every out-of-mapping pair classified with the PO (git wrong / Jira wrong / **real gap** → actionable item in this walk). Ticket-side scope discussion lands in the spec (content home), cited.
...
4. Update single homes: REQUIREMENTS technical statuses for this run's REQ IDs (GATE-2), GAPS, TRACEABILITY (new evidence artifacts).
