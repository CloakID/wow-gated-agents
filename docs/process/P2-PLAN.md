...
1. Run **GATE-6**, **GATE-8** and **GATE-13**, in that order — all mechanical, all cheap:
   ...
   Each unit declares `governs:` — the requirement ids its work updates; declarations (with the spec's `requirements:` header) ARE GATE-2's obligation set (v0.6.4, F-17). A Verify may contain `|` **escaped as `\|`** (v0.6.4, F-18 — an unescaped pipe splits the cell and GATE-8 now names that cause), or route through a file under the unit's evidence dir, which also retains the intermediate output.
