# FORMATS — naming, labels, evidence, status (human view) — DRAFT v0.7.4
...
- **Cross-phase stable IDs (unit-scoped; survive P3→P4→SPEC vN+1):** deviations `DEV-U<n>-<nn>` — **or `DEV-ORCH-<nn>`** (v0.7.3, prodsim/F-66: the ORCH is not a unit and owns decisions no unit can deviate from — branch model, cascade rule, wave boundaries; a playbook deviation had no expressible id and was guaranteed to end as prose no gate reads. ORCH-allocated, single-threaded per run so the F-17 allocator argument holds trivially; carries what/why/impact plus what coverage it gives up, and the PO classifies it at the NEXT gate — mirroring §7's ORCH-authored clause, not reintroducing a human mid-phase) · parks `PARK-U<n>-<nn>` · verifier findings `VF-U<n>-<nn>` · plan defects `DEF-plan-<nn>` · cannot-validate `CV-<run-id>-U<n>-<nn>` (v0.6.3, F-17: the unit segment IS the allocator — unit membership partitions the number space, so parallel verifiers in fresh contexts cannot collide; the legacy run-scoped form without `U<n>` stays valid for existing records) · plan amendments `AM-<nn>` (v0.6.3, F-29: date, decider, what changed, whether it widens scope — **every post-signature modification carries its OWN new `AM-<nn>`**, checked by GATE-9 as a count that must grow across the latest change; v0.7.4, platform/F-71: the first amendment used to license every later silent edit). The block, as a signed artifact carries it: `AM-02 — 2026-09-22 — PO (D-41) — §5 retry budget 3→5; widens scope: no`; a walk row (P4 step 2, v0.7.4) is `| <RUN-REPORT item id> | <classification> | <disposition> |` with the item id whole in the first cell.
...
**Repairing a dead `ev:file` citation — establish what the absence MEANS before acting** (v0.7.4, platform/F-81): a missing target was *renamed* (repoint the citation — `git log --follow -- <path>` shows the move) or *deleted* (restore from history — `git log --diff-filter=D -- <path>` shows the deletion); the two treatments were once applied by one author in one commit, and the wrong one produced a byte-identical phantom copy beside the live file that the derived view counted for a day. A run's files that moved to `runs/archive/<id>/` at P5 are repointed, never restored. At publish, GATE-7 `--p5` refuses while a registry cites a target that exists at neither its path nor the archive rewrite (platform/F-80).
...
**A workflow that lacks a status names its local equivalent** (v0.7.4, platform/F-73 — engine-read, unlike `jira.scope`): a tracker with no Deferred/Backlog made DEFERRED permanently unrepresentable, and the only green path was relabeling the work — the anti-pattern the diff exists to catch. `wow.config.json` → `jira.status_conventions`: `{"deferred_equivalent": "Intake"}` (one key per technical status, `<status>_equivalent`) or `{"equivalents": {"DEFERRED": ["Intake"], "BLOCKED": ["On Hold"]}}`; GATE-10 merges them into this table, so a DEFERRED/Intake pair is expected-consistent with the key set and a divergence without it.
