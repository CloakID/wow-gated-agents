...
- Every commit must carry exactly one lane reference — `[T:<task-id>]` (task work) or `[T:<run-id>]` (a run's phase artifacts), `[Q:…]` (quick), `[D:…]` (debug), `[WOW:publish]` (framework/process publishing), or `[WOW:migrate]` (GSD-migration window only: `.planning/` present, freeze not yet flipped). GATE-1 (commit-msg hook) blocks the rest; backticked/fenced trailers are mentions and do not count. If you find yourself editing files with no active lane, stop and route. Full definitions: `docs/process/LANES.md`.
