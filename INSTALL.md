...
1. Writes/updates the **WoW section** of the target's `CLAUDE.md` (between `<!-- wow-v2:start/end -->` markers; rest of CLAUDE.md untouched). **The marked section is replaced WHOLE** (v0.7.3, platform/F-66): anything a repo adds inside the markers — including recorded PO decisions — is silently reverted on upgrade; repo-local router content lives BELOW the closing marker, and `--check` now shows the first differing lines so a reverted decision is distinguishable from a version bump. The section is passed to the rewrite as a file, never as a regex replacement string — a backslash in the router text used to abort the rewrite (or duplicate the marker block) while the installer reported success.
2. Copies `docs/process/` (playbooks) and `scripts/wow/`: `gates.sh`, **`gates.py`** (the engine gates.sh delegates to), `formats.json`, `status.mjs`, `tests/`, `permissions-policy.json`, and **`GATES-SPEC.md`** (so the repo's CLAUDE.md router can cite a path that exists locally). `scripts/wow/permissions-policy.json` is a **seed file** (v0.6.3, F-06): copied once from the package template, then repo-owned — which commands a repo's verify steps may run is repo-local truth, and shipping it as an engine file meant `--check` called the repo's own policy DRIFTED and the next upgrade overwrote it. `wow.config.json` is created once from a template and never overwritten — it holds repo-local truth: the Jira project key, `migrated_from_gsd`, `requirement_id` (v0.6.1, PF-d — set it when the repo's requirement identities are not `REQ-nnn`, e.g. `"^PLAT-M3-[0-9]{2}$"`, so GATE-2 and status.mjs read the real rows instead of an empty set), and (v0.6.2) `probe_command_pattern` (the dep-probe allowlist override, frisbii S-2) and `main_branch` (the published branch GATE-7 `--p5` compares against, F-11), and (v0.7.1) `jira.scope` (F-42 — a JQL fragment the ORCH ANDs into every mapping query when the Jira project hosts more than this repo's work, e.g. `"component = platform"`; without it GATE-10's divergence diff drowns in tickets this repo never owned. Convention, ORCH-consumed — no engine code reads it, FORMATS §10), and (v0.7.3) `legacy_freeze_exclude` (prodsim/F-60 — fnmatch list carved out of GATE-11's freeze for live runtime paths under the legacy tree; engine-consumed) and `run_base` (prodsim/F-65 — the ref run bases are cut from, default `main_branch` default `main`; convention, ORCH-consumed, with `gates.sh check-id <id> --base <ref>` as its mechanical assert). **Jira hierarchy note** (prodsim/F-61): in a standard team-managed Jira project, `Story` and `Task` are BOTH hierarchy level 0 — siblings — so `task: "Task"` cannot parent-link to a unit Story; set `task: "Subtask"` unless your project runs a premium multi-level hierarchy. Check `getJiraProjectIssueTypesMetadata` before your first G2, not at it.
3. Writes `.claude/commands/wow-*.md` stubs (below) and installs git hooks: **commit-msg** (GATE-1 — message validation cannot run in pre-commit) and **pre-commit** (GATE-5 + GATE-11 + GATE-14 on staged files).
4. Before writing anything, install.sh consults the package's own `docs/GAPS.md`: an open `blocks-install` row whose `scope:` matches the target refuses the install with exit 4, citing the row (`--check` is exempt — reporting drift is never harm). Commit the install itself with the framework-maintenance lane trailer: `git commit -m "chore: install WoW v2 <version> [WOW:publish]"` — `[WOW:publish]` resolves to no task by design (LANES.md) and covers exactly this; do not mix application code into that commit (DEV-R9-03). Idempotent; re-run to upgrade. Version stamp in `wow.config.json` is **re-stamped by every install** — it is the one key the installer owns in the repo-owned file (DEV-R9-04); `install.sh --check` diffs installed vs canonical and reports a stale stamp. **A CLAUDE.md wow-v2 section carrying repo-local edits is REFUSED by a plain install** with the would-be-lost lines shown; move repo-local lines below the closing marker, or pass `--force-section` to discard them deliberately (DEV-R9-01). The installer also seeds an empty-but-valid `docs/GAPS.md` so the first `/wow-spec` does not dead-end on GATE-12 (DEV-R9-02).

**Upgrades carry no migration paths — by policy** (PO decision 2026-09-03, closing OBL-PKG-14). This is a shared open-source package: it guarantees that a format mismatch in a consuming repo's durable homes is **seen loudly** (the FR-1 family — unreadable registries, rows, plans and maps all refuse rather than parse as empty), and it does not write per-consumer row-migration recipes. Reconciling local artifacts to a new format is the consuming repo's work, done deliberately in its own lanes (frisbii's ADR-009 borrow-named-fixes pattern is one good model). A repo that upgrades and goes red has been told exactly what to reconcile — that is the contract.
...
scripts/wow/gates.sh check-id <run-id>           # run open (P1): refuse an inexpressible id early (F-46)
